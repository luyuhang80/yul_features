...
class InputHelper():
	...
	def transform_raw(self, text, sequence_length):
		...
		x = [self.token_dictionary.get(token) for token in text.split() if token in self.token_dictionary]
		w = [token for token in text.split() if token in self.token_dictionary]
		x = x[:sequence_length]
		w = w[:sequence_length]
		padding_index = self.vocab_size - 1
