...
class BiRNN(object):
	...
	def __init__(self, embedding_size, rnn_size, layer_size, 
		...
		with tf.name_scope('bi_rnn'), tf.variable_scope('bi_rnn'):
			self.outputs, _, _ =  tf.contrib.rnn.static_bidirectional_rnn(lstm_fw_cell_m, lstm_bw_cell_m, inputs, dtype=tf.float32)
		...
		with tf.name_scope('attention'), tf.variable_scope('attention'):
			...
			self.final_output = tf.reduce_sum(self.outputs * alpha_trans, 0)
	...
	def inference(self, sess, labels, inputs):
		...
		prob,out_features = sess.run([self.prob,self.outputs], feed_dict={self.input_data:inputs, self.output_keep_prob:1.0})
		ret = np.argmax(prob, 1)
		ret = [labels[i] for i in ret]
		return ret , out_features
